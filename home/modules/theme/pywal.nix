...
let
  ...
  fallbackNiriTheme = pkgs.writeText "niri-wal-fallback" ''
    layout {
        focus-ring {
            active-color "#a9c7ff"
            inactive-color "#1b1b1f"
        }
        border {
            active-color "#a9c7ff"
            inactive-color "#938f99"
        }
        shadow {
            color "#1b1b1fa6"
        }
  ...
  fallbackMakoTheme = pkgs.writeText "mako-wal-fallback" ''
    # Fallback Mako Notification Config
    max-history=100
    sort=-time
    layer=overlay
    anchor=top-right
  ...
  walColorExport = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      NIRI_COLORS="$HOME/.config/niri/wal-colors.kdl"
...
EOF
      ...
      # Niri sources this file and watches it for live reloading
      if atomic_write "$NIRI_COLORS" <<EOF
...
EOF
      ...
      if atomic_write "$MAKO_CONFIG" <<EOF
# Generated by wal-color-export. Manual edits will be replaced.
max-history=100
sort=-time
layer=overlay
...
EOF
      ...
      # Reload mako before sending theme update notification
...
in
{
  ...
  home.activation.ensureRuntimePaletteFiles = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    niri_theme="$HOME/.config/niri/wal-colors.kdl"
    if [[ ! -e "$niri_theme" ]]; then
      run mkdir -p "$(dirname "$niri_theme")"
      run install -m 0644 ${fallbackNiriTheme} "$niri_theme"
    fi

    mako_config="$HOME/.config/mako/config"
    if [[ ! -e "$mako_config" ]]; then
      run mkdir -p "$(dirname "$mako_config")"
      run install -m 0644 ${fallbackMakoTheme} "$mako_config"
    fi
