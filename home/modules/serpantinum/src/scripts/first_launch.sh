...
if [ -n "$WP_DIR" ] && [ -d "$WP_DIR" ]; then
    ...
    if [ -n "$RANDOM_WP" ]; then
        quickshell ipc -p "$MAIN_QML" call wallpaper setWallpaper "all" "$RANDOM_WP" "fade" >/dev/null 2>&1 || true

        quickshell ipc -p "$MAIN_QML" call matugen generate "$RANDOM_WP" "" "" >/dev/null 2>&1 || quickshell ipc -p "$MAIN_QML" call matugen generate "$RANDOM_WP" >/dev/null 2>&1 || true
