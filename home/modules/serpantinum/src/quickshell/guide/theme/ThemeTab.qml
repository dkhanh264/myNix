    ...
    Process {
        ...
        command: {
            ...
            let pyScript =
                ...
                "    sys.exit(0)\n" +
                "try:\n" +
                "    p = subprocess.run(['quickshell', 'ipc', '-p', main_qml, 'call', 'wallpaper', 'getWallpaperPath', screen], capture_output=True, text=True)\n" +
                "    val = p.stdout.strip()\n" +
                "except Exception:\n" +
                "    val = ''\n" +
                "if not val:\n" +
                "    try:\n" +
                "        p = subprocess.run(['quickshell', 'ipc', '-p', main_qml, 'call', 'wallpaper', 'getWallpaper', screen], capture_output=True, text=True)\n" +
                "        val = p.stdout.strip()\n" +
                "    except Exception:\n" +
                "        val = ''\n" +
                "if not val:\n" +
            ...
            onStreamFinished: {
                ...
                if (out !== "") {
                    ...
                    if (idx !== -1) {
                        ...
                        if (mainQmlPath !== "") {
                            let escapeBash = function(str) { return String(str).replace(/(["\\$`])/g, '\\$1'); }
                            Quickshell.execDetached(["bash", "-c", "quickshell ipc -p \"" + escapeBash(mainQmlPath) + "\" call fonts rescan"]);
                        }
