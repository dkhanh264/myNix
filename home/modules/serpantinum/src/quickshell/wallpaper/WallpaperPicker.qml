...
Item {
    ...
    function setWallpaperOnMonitors(targetFile, transition) {
        ...
        if (outputs === "all") {
            Quickshell.execDetached(["quickshell", "ipc", "-p", Caching.mainQml, "call", "wallpaper", "setWallpaper", "all", targetFile, transition]);
        } else {
            let monArr = outputs.split(",");
            for (let i = 0; i < monArr.length; i++) {
                Quickshell.execDetached(["quickshell", "ipc", "-p", Caching.mainQml, "call", "wallpaper", "setWallpaper", monArr[i], targetFile, transition]);
            }
        }
    }
