...
PanelWindow {
    ...
    function sendIpc(funcName, args) {
        let cmd = ["quickshell", "ipc", "-p", Caching.mainQml, "call", "widgets-" + redactorWindow.safeMonitorName, funcName];
    ...
    function sendBarIpc(funcName, args) {
        let cmd = ["quickshell", "ipc", "-p", Caching.mainQml, "call", "topbar", funcName];
        if (args && args.length > 0) {
            cmd = cmd.concat(args);
        }
        Quickshell.execDetached(cmd);
                ...
                delegate: Item {
                    ...
                    Timer {
                        ...
                        onTriggered: {
                            ...
                            let cmd = ["quickshell", "ipc", "-p", Caching.mainQml, "call", "widgets-" + redactorWindow.safeMonitorName, "geometry", String(widgetProxy.wId), model.wX.toString(), model.wY.toString(), model.wWidth.toString(), model.wHeight.toString(), curOp.toString()];
